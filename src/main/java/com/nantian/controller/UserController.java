...
public class UserController {
    ...
    public void getUser(ApplicationContext context, String username) {
        if (StringUtils.isBlank(username)) {
            context.setErrorMsg(ErrorCodeEnum.PARAMETER_ERROR.getErrcode(), "要查询的用户不能为空");
            return;
        }
...
//        }
    ...
    public void queryByIds(ApplicationContext context,@ParameterObject UserVO userVO){
        ...
        for(int i = 0 ;i<userVO.getRoleIds().size();i++){
            Long id = userVO.getRoleIds().get(i);
    ...
    @Push(requestFormat = ParamFormatEnum.JSON)
    public void queryById(ApplicationContext context,@ParameterObject ListFlowVO listFlowVO) {

        System.out.println(listFlowVO+"*******");
